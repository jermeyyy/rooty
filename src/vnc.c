...
static struct n_subprocess_info *vnc_sub_info;

static pid_t vnc_pid;
int isVNCDrunning = 0;

static char *vnc_argv[] = { "/sbin/vncd", NULL, NULL };
...
int init_vnc(void)
{
    ...
    vnc_sub_info = n_call_usermodehelper_setup(
        vnc_argv[0], vnc_argv, vnc_envp, GFP_ATOMIC);
    if (vnc_sub_info == NULL)
        return -ENOMEM;

    ret = n_call_usermodehelper("/sbin/vncd", vnc_argv, vnc_envp,
                                UMH_WAIT_EXEC);
    if (ret != 0) {
